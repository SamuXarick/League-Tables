function LeagueTable::GetRoadInfrastructureEfficiency_Val(company)
{
	local rating = 0;
	local score = SetText(GetRoadInfrastructureEfficiency_ScoreString(), [ 0, 0, 0, 0 ]);
	local element = SetText(GSText.STR_ROAD_INFRASTRUCTURE_EFFICIENCY_NONE, []);
	...
	if (GSCompany.ResolveCompanyID(company) != GSCompany.COMPANY_INVALID) {
		...
		if (GSStation.IsValidStation(best_rated_station)) {
			...
			element.str = GetRoadInfrastructureEfficiency_ElementString();
			element.p = [ SetText(station_string), best_rated_station, highest_cargo_rating ];
...
function LeagueTable::GetRoadInfrastructureEfficiency_ScoreString()
{
	return GSText.STR_ROAD_INFRASTRUCTURE_EFFICIENCY_SCORE;
}

function LeagueTable::GetRoadInfrastructureEfficiency_TitleString()
{
	return GSText.STR_ROAD_INFRASTRUCTURE_EFFICIENCY_TITLE;
}

function LeagueTable::GetRoadInfrastructureEfficiency_HeaderString()
{
	return GSText.STR_ROAD_INFRASTRUCTURE_EFFICIENCY_HEADER;
}

function LeagueTable::GetRoadInfrastructureEfficiency_FooterString()
{
	return GSText.STR_ROAD_INFRASTRUCTURE_EFFICIENCY_FOOTER;
}

function LeagueTable::GetRoadInfrastructureEfficiency_ElementString()
{
