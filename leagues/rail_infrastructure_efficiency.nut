function LeagueTables::GetRailInfrastructureEfficiency_Stats(company)
{
	local rating = 0;
	local score_text = SetText(GetRailInfrastructureEfficiency_ScoreString(), [ 0, 0, 0, 0 ]);
	local element_text = SetText(GSText.STR_RAIL_INFRASTRUCTURE_EFFICIENCY_NONE);
	...
	if (GSCompany.ResolveCompanyID(company) != GSCompany.COMPANY_INVALID) {
		...
		if (GSStation.IsValidStation(best_rated_station)) {
			...
			element_text.string = GetRailInfrastructureEfficiency_ElementString();
...
function LeagueTables::GetRailInfrastructureEfficiency_ScoreString()
{
	return GSText.STR_RAIL_INFRASTRUCTURE_EFFICIENCY_SCORE;
}

function LeagueTables::GetRailInfrastructureEfficiency_TitleString()
{
	return GSText.STR_RAIL_INFRASTRUCTURE_EFFICIENCY_TITLE;
}

function LeagueTables::GetRailInfrastructureEfficiency_HeaderString()
{
	return GSText.STR_RAIL_INFRASTRUCTURE_EFFICIENCY_HEADER;
}

function LeagueTables::GetRailInfrastructureEfficiency_FooterString()
{
	return GSText.STR_RAIL_INFRASTRUCTURE_EFFICIENCY_FOOTER;
}

function LeagueTables::GetRailInfrastructureEfficiency_ElementString()
{
