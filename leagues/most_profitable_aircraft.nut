function LeagueTables::GetMostProfitableAircraft_Stats(company)
{
	local rating = 0;
	local score_text = SetText(GetMostProfitableAircraft_ScoreString(), [ 0, 0, 0, 0 ]);
	local element_text = SetText(GSText.STR_MOST_PROFITABLE_AIRCRAFT_NONE, [ -1 ]);
	...
	if (GSCompany.ResolveCompanyID(company) != GSCompany.COMPANY_INVALID) {
		...
		if (!vehicle_list.IsEmpty()) {
			...
			element_text = SetText(GetMostProfitableAircraft_ElementString(), [ vehicle ]);
...
function LeagueTables::GetMostProfitableAircraft_ScoreString()
{
	return GSText.STR_MOST_PROFITABLE_AIRCRAFT_SCORE;
}

function LeagueTables::GetMostProfitableAircraft_TitleString()
{
	return GSText.STR_MOST_PROFITABLE_AIRCRAFT_TITLE;
}

function LeagueTables::GetMostProfitableAircraft_HeaderString()
{
	return GSText.STR_MOST_PROFITABLE_AIRCRAFT_HEADER;
}

function LeagueTables::GetMostProfitableAircraft_FooterString()
{
	return GSText.STR_MOST_PROFITABLE_AIRCRAFT_FOOTER;
}

function LeagueTables::GetMostProfitableAircraft_ElementString()
{
